...
Miro Samek的现代嵌入式编程课程第7课重点讲解此工作原理，深入剖析ARM汇编代码如何实现数组索引寻址，以及这种语法糖背后的硬件机制。
...
- **数据寄存器**：偏移量为 `0x000`
- **位屏蔽技巧**：地址偏移中的位 `[9:2]` 作为"掩码"，写入数据的低8位是实际输出值
...
- 由地址偏移恢复掩码：`56 >> 2 = 14`，即 `00001110`，表示要修改LED引脚（位1、2、3）
- 使用低8位的值（0）覆盖这些位
...
```c
volatile unsigned long *gpio_bits = (volatile unsigned long *)(0x40025000 + 0x000);
```
...
这里 `gpio_bits` 是 32 位指针，因此 `gpio_bits + LED_RED` 的地址增量是 `LED_RED * 4`，恰好对应位屏蔽地址窗口。编译器会生成等价的寻址代码。
